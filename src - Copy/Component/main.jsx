...
const Main = (props) => {
    ...
    const addToList = () => {
        if (!inputData) {
            ...
            let todo = {id: id , text: inputData, completed: false , status : 'active'}
    ...
    const changeClass = (idx,item)=>{
        ...
        let updatedItemsList = items.map((todo) => {
            if(todo.id === idx) { todo.status = 'completed' }
            return todo
        })

        addItems([...updatedItemsList]);

        // console.clear(); console.log(updatedItemsList)

        // return void 0;
       
          updatefilter(updatedItemsList)
          savedata(item)
          completedItems(division)
    }
    ...
    useEffect(()=> { console.log('filter state changed',filter); },[filter])
        ...
        <div className="container">
            <div className="main-center">
                ...
                <section className="main-contain">
                    ...
                    { division == "active"    &&  <ActiveScreen deta ={items} fun={changeClass} dlt={deleteItem}/> }
                    { division == "completed" &&  <CompletedScreen dota = {items} fun= {changeClass} dlt={deleteItem} />}
                    { division == "all"       &&  <AllScreen data = {items} fun= {changeClass} dlt={deleteItem} />}
